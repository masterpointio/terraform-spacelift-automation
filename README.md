...
- Stack configs may be grouped into subdirectories under `stacks/`, which is useful for single-tenant infrastructure where one root module is deployed per tenant, e.g. `stacks/acme/prod-acme.yaml` and `stacks/globex/prod-globex.yaml`. The subdirectory is organizational only — the workspace name comes from the file name alone (`prod-acme`), so a stack keeps its name if it is later moved between subdirectories. Because the subdirectory is dropped from the name, stack file base names must be unique within a root module. The `tfvars/` directory mirrors the `stacks/` layout, so this example pairs with `tfvars/acme/prod-acme.tfvars`, and the Spacelift folder label becomes `folder:<root_module>/acme/prod-acme`.

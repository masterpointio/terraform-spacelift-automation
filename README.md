...
| <a name="input_project_root_prefix"></a> [project\_root\_prefix](#input\_project\_root\_prefix) | A prefix to prepend to the calculated project\_root for all stacks.<br/>This is useful when root\_modules\_path uses relative paths (e.g., "../../root-modules" for local scanning)<br/>but the actual path in the repository is different (e.g., "terraform/root-modules").<br/>When set, the default project\_root becomes: project\_root\_prefix/module\_name<br/>Per-stack project\_root in YAML files still takes precedence over this. | `string` | `null` | no |

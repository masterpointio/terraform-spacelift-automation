...
3. [Spacelift AWS Integration Attachment](https://docs.spacelift.io/integrations/cloud-providers/aws#lets-explain)
   ...
   - The default setup is a single integration: set `aws_integration_id` (or `aws_integration_name`) and it's used for all TF executions (further `assume_role` can be done in the TF `provider` block.). If least-privilege is important for security, and you want read-only operations (such as proposed PR Terraform plans) to run with a read-only IAM role while `apply`/`destroy` uses a separate read/write role, you can opt into [split read/write attachments](https://spacelift.io/blog/spacelift-cloud-integrations#read-vs-write-attachments) by setting both `aws_integration_read_*` and `aws_integration_write_*`.
...
- Stack configs may be grouped into subdirectories under `stacks/`, e.g. `stacks/acme/prod-acme.yaml`. This is useful for single-tenant infrastructure, where one root module is deployed per tenant and each tenant has several environments.
  - The `tfvars/` directory mirrors the `stacks/` layout, so this example pairs with `tfvars/acme/prod-acme.tfvars`, and the Spacelift folder label becomes `folder:<root_module>/acme/prod-acme`.
  - The subdirectory is organizational only: the workspace name comes from the file name alone (`prod-acme`), so stack file base names must be unique within a root module.
...
- Common configuration is defined in `root-modules/random-pet/stacks/common.yaml` and applied to both `random-pet-dev` and `random-pet-prod` stacks. A Stack config (e.g. `root-modules/random-pet/stacks/dev.yaml`) is deep merged on top of it — a single value like `autodeploy` is replaced, but a list like `labels` is combined. See [How are configurations merged from multiple files, and which values win?](#how-are-configurations-merged-from-multiple-files-and-which-values-win) for details.
...
### How are configurations merged from multiple files, and which values win?
...
`random-pet-dev` ends up with `labels: ["team:platform", "env:dev", "repository:acme-infra"]` — all three kept — and `autodeploy: true`, since `dev.yaml` replaces the `common.yaml` value. Every list behaves like `labels`, including the `before_*` / `after_*` hooks.

> [!IMPORTANT]
> Because lists append, a label or hook inherited from `common.yaml` **cannot be removed** at the stack level. Only put values there that apply to every stack in the root module.

**Module inputs to `spacelift-automation`** (`var.*`) are the one layer not ordered by precedence. `var.labels` and hook lists like `var.before_init` always append, so they apply to every stack the module manages, while single values like `var.autodeploy` are only defaults that any YAML value overrides.
...
| <a name="input_aws_integration_id"></a> [aws\_integration\_id](#input\_aws\_integration\_id) | ID of the AWS integration to use for both read and write operations. Use this when a single integration covers both sides. For separate per-side integrations, use aws\_integration\_read\_id and aws\_integration\_write\_id instead — do not combine with the per-side variables. | `string` | `null` | no |
| <a name="input_aws_integration_name"></a> [aws\_integration\_name](#input\_aws\_integration\_name) | Name of the AWS integration to attach for both read and write operations, resolved to an ID. Mutually exclusive with aws\_integration\_id. We recommend using names rather than IDs to improve clarity & readability. Since Spacelift enforces unique names, you can rely on names as identifiers without worrying about duplication issues. | `string` | `null` | no |
| <a name="input_aws_integration_read_id"></a> [aws\_integration\_read\_id](#input\_aws\_integration\_read\_id) | ID of the AWS integration to attach for read operations. Use together with aws\_integration\_write\_id when different integrations are needed per side. Mutually exclusive with aws\_integration\_id — do not set both. | `string` | `null` | no |
| <a name="input_aws_integration_read_name"></a> [aws\_integration\_read\_name](#input\_aws\_integration\_read\_name) | Name of the AWS integration to attach for read operations only, resolved to an ID. Mutually exclusive with aws\_integration\_read\_id. | `string` | `null` | no |
| <a name="input_aws_integration_write_id"></a> [aws\_integration\_write\_id](#input\_aws\_integration\_write\_id) | ID of the AWS integration to attach for write operations. Use together with aws\_integration\_read\_id when different integrations are needed per side. Mutually exclusive with aws\_integration\_id — do not set both. | `string` | `null` | no |
| <a name="input_aws_integration_write_name"></a> [aws\_integration\_write\_name](#input\_aws\_integration\_write\_name) | Name of the AWS integration to attach for write operations only, resolved to an ID. Mutually exclusive with aws\_integration\_write\_id. | `string` | `null` | no |
...
| <a name="input_dependency_labels_enabled"></a> [dependency\_labels\_enabled](#input\_dependency\_labels\_enabled) | Whether to automatically add `depends-on` labels to stacks. When enabled, each stack gets a `depends-on:<automation-stack-name>` label. You may set this to `false` if you do NOT want stacks to be triggered after the automation stack finishes running, or have a custom trigger policy in place that supports this. | `bool` | `true` | no |
...
| <a name="input_enable_local_preview"></a> [enable\_local\_preview](#input\_enable\_local\_preview) | Indicates whether local preview runs can be triggered on this Stack. | `bool` | `true` | no |
...
| <a name="input_git_sparse_checkout_paths"></a> [git\_sparse\_checkout\_paths](#input\_git\_sparse\_checkout\_paths) | List of paths used for Git sparse checkout. When set, only the specified paths are fetched from the repository during a run. This is particularly useful for large repositories where you only need to work with specific directories or files. | `list(string)` | `null` | no |
...
| <a name="input_labels"></a> [labels](#input\_labels) | List of labels to apply to the stacks. These are deep merged (appended) with the common configs (along with the other lists in the stack config YAML files). | `list(string)` | `[]` | no |
...
| <a name="input_project_root_prefix"></a> [project\_root\_prefix](#input\_project\_root\_prefix) | Repo-root-relative path joined with each module name to form that stack's Spacelift<br/>project\_root. e.g. "root-modules" produces project\_root = "root-modules/network" for<br/>the network module.<br/><br/>If null, falls back to root\_modules\_discovery\_path verbatim — only valid when that<br/>path is already repo-root-relative (no "../"). Per-stack stack\_settings.project\_root<br/>in YAML always wins. | `string` | `null` | no |
...
| <a name="input_root_modules_discovery_path"></a> [root\_modules\_discovery\_path](#input\_root\_modules\_discovery\_path) | Directory that spacelift-automation scans during plan/apply to discover stack YAML<br/>files (`<module>/stacks/**/*.yaml` for MultiInstance, `<module>/stack.yaml` for<br/>SingleInstance), creating one Spacelift Stack per file found. Subdirectories under<br/>`stacks/` are organizational only: they group files in Git and in the Spacelift UI<br/>folder label, but do not contribute to the stack or workspace name, so stack file<br/>base names must be unique within a root module. Resolved relative to<br/>the consuming root module (path.root); it does not affect the project\_root of<br/>generated stacks — set project\_root\_prefix for that.<br/><br/>Example: with this module at <repo>/root-modules/spacelift-automation/ and sibling<br/>root modules at <repo>/root-modules/<module>/, use "../". | `string` | `"../"` | no |

...
| <a name="input_workspace_as_stack_name_prefix"></a> [workspace\_as\_stack\_name\_prefix](#input\_workspace\_as\_stack\_name\_prefix) | Controls the order of components in MultiInstance stack IDs.<br/>- `false` (default): `${module}-${workspace}` (e.g. `network-dev`)<br/>- `true`: `${workspace}-${module}` (e.g. `dev-network`)<br/><br/>Ignored for SingleInstance (stack ID is always `${module}`).<br/>Changing this renames stack IDs, forcing recreation of Spacelift stacks. Set deliberately. | `bool` | `false` | no |

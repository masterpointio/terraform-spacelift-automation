# locals {
#   root_modules_path    = "components"
#   enabled_root_modules = ["spacelift-policies"] #, "spacelift-automation"]
...
#       for yaml_file in fileset("${path.root}/${local.root_modules_path}/${module}/stacks", "*.yaml") :
#       yaml_file => yamldecode(file("${path.root}/${local.root_modules_path}/${module}/stacks/${yaml_file}"))
#     }
#   }
...
#   stacks_content = merge([for module, files in local.root_module_yaml_decoded : {
...
#   version  = "1.0.2"
#   for_each = local.stacks_content
#   maps     = [each.value, local.common_content[each.value.root_module]]
# }
...
module "automation" {
  ...
  root_modules_path    = "components"
  enabled_root_modules = ["spacelift-policies"]
...
#       ["cp tfvars/${local.stack_configs[stack].terraform_workspace}.tfvars spacelift.auto.tfvars"],
#     ))
#   }
# }
...
#   space_id                     = coalesce(try(local.stack_configs[each.key].space_id, null), var.stack_space_id)
#   name                         = each.key
#   administrative               = coalesce(try(local.stack_configs[each.key].administrative, null), var.stack_administrative)
#   after_apply                  = compact(concat(try(local.stack_configs[each.key].after_apply, []), var.stack_after_apply))
#   after_destroy                = compact(concat(try(local.stack_configs[each.key].after_destroy, []), var.stack_after_destroy))
#   after_init                   = compact(concat(try(local.stack_configs[each.key].after_init, []), var.stack_after_init))
#   after_perform                = compact(concat(try(local.stack_configs[each.key].after_perform, []), var.stack_after_perform))
#   after_plan                   = compact(concat(try(local.stack_configs[each.key].after_plan, []), var.stack_after_plan))
#   autodeploy                   = coalesce(try(local.stack_configs[each.key].autodeploy, null), var.stack_autodeploy)
#   autoretry                    = try(local.stack_configs[each.key].autoretry, var.stack_autoretry)
#   before_apply                 = compact(coalesce(try(local.stack_configs[each.key].before_apply, []), var.stack_before_apply))
#   before_destroy               = compact(coalesce(try(local.stack_configs[each.key].before_destroy, []), var.stack_before_destroy))
#   before_init                  = local.before_init[each.key]
#   before_perform               = compact(coalesce(try(local.stack_configs[each.key].before_perform, []), var.stack_before_perform))
#   before_plan                  = compact(coalesce(try(local.stack_configs[each.key].before_plan, []), var.stack_before_plan))
#   description                  = coalesce(try(local.stack_configs[each.key].description, null), var.stack_description)
...
#   project_root                 = local.stack_configs[each.key].project_root
#   manage_state                 = try(local.stack_configs[each.key].manage_state, var.stack_manage_state)
#   labels                       = local.labels[each.key]
#   enable_local_preview         = try(local.stack_configs[each.key].enable_local_preview, var.stack_enable_local_preview)
#   terraform_smart_sanitization = try(local.stack_configs[each.key].terraform_smart_sanitization, var.stack_terraform_smart_sanitization)
#   terraform_version            = try(local.stack_configs[each.key].terraform_version, var.stack_terraform_version)
#   terraform_workflow_tool      = var.terraform_workflow_tool
#   terraform_workspace          = local.stack_configs[each.key].terraform_workspace

#   protect_from_deletion = try(local.stack_configs[each.key].protect_from_deletion, var.stack_protect_from_deletion)

#   worker_pool_id = try(local.stack_configs[each.key].worker_pool_id, var.stack_worker_pool_id)
...
# # Don't toggle the creation/destruction of this resource with var.stack_destructor_enabled,
...
#   deactivated = !try(local.stack_configs[each.key].destructor_enabled, var.stack_destructor_enabled)
...
#     if try(local.stack_configs[key].drift_detection_enabled, var.stack_drift_detection_enabled)
#   } : {}

#   stack_id     = spacelift_stack.this[each.key].id
#   ignore_state = try(local.stack_configs[each.key].drift_detection_ignore_state, var.stack_drift_detection_ignore_state)
#   reconcile    = try(local.stack_configs[each.key].drift_detection_reconcile, var.stack_drift_detection_reconcile)
#   schedule     = try(local.stack_configs[each.key].drift_detection_schedule, var.stack_drift_detection_schedule)
#   timezone     = try(local.stack_configs[each.key].drift_detection_timezone, var.stack_drift_detection_timezone)
# }
